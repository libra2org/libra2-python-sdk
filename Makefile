...
test:
	poetry run python -m unittest discover -s libra2_sdk/ -p '*.py' -t ..
	poetry run behave

test-coverage:
	poetry run python -m coverage run -m unittest discover -s libra2_sdk/ -p '*.py' -t ..
...
fmt:
	find ./examples ./libra2_sdk ./features . -type f -name "*.py" | xargs poetry run autoflake -i -r --remove-all-unused-imports --remove-unused-variables --ignore-init-module-imports
	poetry run isort libra2_sdk examples features
	poetry run black libra2_sdk examples features

lint:
	poetry run mypy libra2_sdk examples features
	poetry run flake8 libra2_sdk examples features

examples:
	poetry run python -m examples.libra2_token
	...
	poetry run python -m examples.simple_libra2_token
...
examples_cli:
	...
	#poetry run python -m examples.multisig CURRENTLY BROKEN requires libra2-core checkout
