...
DEFAULT_BINARY = os.getenv("LIBRA2_CLI_PATH", "libra2")
...
class Libra2CLIWrapper:
    """Tooling to make easy access to the Libra2 CLI tool from within Python."""
    ...
    def compile_package(package_dir: str, named_addresses: Dict[str, AccountAddress]):
        Libra2CLIWrapper.assert_cli_exists()
        args = [
            DEFAULT_BINARY,
            "move",
            "compile",
        ...
        args.extend(Libra2CLIWrapper.prepare_named_addresses(named_addresses))
    ...
    @staticmethod
    def start_node() -> Libra2Instance:
        Libra2CLIWrapper.assert_cli_exists()
        return Libra2Instance.start()
    ...
    def assert_cli_exists():
        if not Libra2CLIWrapper.does_cli_exist():
...
class Libra2Instance:
    """
    A standalone Libra2 node running by itself. This still needs a bit of work:
    * a test instance should be loaded into its own port space. Currently they share ports as
      ...
      test runs may share a single Libra2Instance and both successfully complete.
    * Probably need some means to monitor the process in case it stops, as we aren't actively
    ...
    def start() -> Libra2Instance:
        ...
        return Libra2Instance(node_runner, temp_dir)
